...
export default {
    ...
    return {
      ...
      col: [
        ...
        },
        {
          ...
          // key:'payAmountArr',
          render: (h, params) => {
            if (this.payAmountArr[params.index] == null) {
                this.payAmountArr[params.index] = "0.00";
            }
            return h("span", this.payAmountArr[params.index]);
          }
        },
        {
          title: "支付方式",
          render: (h, params) => {
            const _index = this.payList.findIndex(
              item => item.id === params.row.paymentMethod
            );
            if (_index >= 0) {
        ...
        // },
        {
          title: "订单状态",
          render: (h, params) => {
            const _index = this.typeList.findIndex(
              item => item.id === params.row.orderStatus
            );
            if (_index >= 0) {
              return h("span", this.typeList[_index].name);
            }
  ...
  methods: {
      ...
      _vm.$http.get({
        ...
        success: function(res) {
          if (res.code === 0) {
            ...
            var payAmount = res.content.content;
            _vm.payAmountArr = payAmount.map(item => item.payAmount);
            console.log(_vm.payAmountArr);
          } else {
    ...
    refresh() {
      ...
      (this.formSearch.startTime = moment().startOf("month")),
        (this.formSearch.endTime = moment().format("YYYY-MM-DD") + " 23:59:59"),
        (this.timeBox = []);
