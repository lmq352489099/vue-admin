<template>
  <div class='tinymce-container editor-container'>
    ...
    <p style='textAlign:right'>剩余{{countNum}}字</p>
...
export default {
  ...
  watch: {
    value(val) {
      console.log(val)
      if(val === '<p><br data-mce-bogus="1"></p>' || val === '<p><br></p>') {
        this.num = 0
      } else {
        this.num = val.length
      }
      this.$emit('valid', this.num)
  ...
  methods: {
    imageSuccessCBK(arr) {
      const _this = this
      arr.forEach(v => {
        window.tinymce.get(_this.tinymceId).insertContent(`<img src="${v}" >`)
      })
    }
  },
  destroyed() {
